...
public class Server extends io.stargate.proto.StargateGrpc.StargateImplBase {
  ...
  public static final Context.Key<AuthenticationSubject> AUTH_KEY = Context.key("auth");
  ...
  public Server(Persistence persistence) {
  ...
  public void execute(Query query, StreamObserver<Result> responseObserver) {
    try {
      AuthenticationSubject authenticationSubject = AUTH_KEY.get();
  ...
  private void prepare(
    ...
    connection
        .loggedUser()
        .ifPresent(
            user -> keyBuilder.append(user.name()));
